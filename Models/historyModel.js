import mongoose from 'mongoose';
import { 
    METASTASIS,
    ETAPA_CANCER,
    SENO,
    CUADRANTE_SENO,
    HISTORIAL,
    NODULOS_LINFATICOS,
    GENERO
 } from '../Utils/Constants';

const historySchema = new mongoose.Schema(
    {
        
        Metastasis: {
            type:String,
            enum: Object.values(METASTASIS),
        },
        Cancer_Stage: {
            type:String,
            enum: Object.values(ETAPA_CANCER)
        },
        radius_mean: Number,
        texture_mean: Number,
        perimeter_mean: Number,
        area_mean: Number,
        smoothness_mea: Number,
        compactness_mean: Number,
        concavity_mean: Number,
        "concave points_mean": Number,
        symmetry_mean: Number,
        fractal_dimension_mean: Number,
        radius_se: Number,
        texture_se: Number,
        perimeter_se: Number,
        area_se: Number,
        smoothness_se: Number,
        compactness_se: Number,
        concavity_se: Number,
        "concave points_se": Number,
        symmetry_se: Number,
        fractal_dimension_se: Number,
        radius_worst: Number,
        texture_worst: Number,
        perimeter_worst: Number,
        area_worst: Number,
        smoothness_worst: Number,
        compactness_worst: Number,
        concavity_worst: Number,
        "concave points_worst": Number,
        symmetry_worst: Number,
        fractal_dimension_worst: Number,
        "Tumor Size (cm)": Number,
        Breast: {
            type:String,
            enum: Object.values(SENO)
        },
        "Breast Quadrant": {
            type:String,
            enum: Object.values(CUADRANTE_SENO)
        },
        History: {
            type:String,
            enum: Object.values(HISTORIAL)
        },
        "Inv-Nodes": {
            type:String,
            enum:Object.values(NODULOS_LINFATICOS)
        },
        Menopause: Number,
        Age: Number,
        Gender: {
            type:String,
            enum: Object.values(GENERO)
        },
        Genetic_Risk: Number,
        Country_Region: Number,
        Air_Pollution: Number,
        Alcohol_Use: Number,
        Smoking: Number,
        Obesity_Level: Number

    
    }
);

userSchema.methods.toJSON = function(){
    let obj = this.toObject()
    delete obj.password
    return obj;
}

export default mongoose.model('History', historySchema);